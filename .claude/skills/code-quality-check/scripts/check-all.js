...
if (detectEslint()) {
  ...
  if (eslintResult.success && results.eslint.errors === 0) {
    ...
    if (shouldFix) {
      ...
      log(
        '[!]',
        `Fixed what was auto-fixable. Remaining: ${results.eslint.errors} errors, ${results.eslint.warnings} warnings`
      )
    } else {
...
console.log(
  `Prettier:   ${statusEmoji[results.prettier.status]} ${results.prettier.status.toUpperCase()}`
)
console.log(
  `ESLint:     ${statusEmoji[results.eslint.status]} ${results.eslint.status.toUpperCase()} (${results.eslint.errors} errors, ${results.eslint.warnings} warnings)`
)
console.log(
  `TypeScript: ${statusEmoji[results.typescript.status]} ${results.typescript.status.toUpperCase()} (${results.typescript.errors} errors)`
)
if (!skipTests) {
  console.log(
    `Tests:      ${statusEmoji[results.tests.status]} ${results.tests.status.toUpperCase()}`
  )
}
